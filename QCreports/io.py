...
from QCreports.generate_worksheets import report_definition_1wy
...
def focus_report_data(zones, input_file_name="report_data.xlsx", output_file_name="report_data_focus.xlsx"):
...
def generate_QCplots(input_file_name="report_data.xlsx", output_file_name="report.xlsx"):
    xls = pd.ExcelFile(input_file_name)
...
def generate_QCmaps(years, input_file_name="report_data.xlsx", output_file_name="report.png"):
    xls = pd.ExcelFile(input_file_name)
    sheet_names = xls.sheet_names
    ...
    fig.savefig(output_file_name, format="png", bbox_inches="tight")

def generate_report_1wy(tags, years):
    worksheets = report_definition_1wy(tags)
    make_report_data(worksheets)
    generate_QCplots()
    generate_QCmaps(years)
