...
def report_definition_1wy(tags):

    ids = cgriddb.get_jobid(comment=tags)
    worksheets = []

    sheet_name = "price"
    df = cgriddb.get_value_with_index(ids, 'zone,mean_price', 'my')
    ...
    df = cgriddb.get_value_with_index(ids, 'zone,price', 'my')
    ...
    df = cgriddb.get_value_with_index(ids, 'resource,capture_rate,^wind_onshore.*', 'my')
    df = df.ffill().bfill()
    ...
    df = cgriddb.get_value_with_index(ids, 'resource,capture_rate,^wind_offshore.*', 'my')
    df = df.ffill().bfill()
    columns = [column for column in df.columns if "existing" not in column]
    df = df[columns]
    df.columns = [column.rsplit("_", 1)[1] for column in df.columns]
    ...
    df = cgriddb.get_value_with_index(ids, 'resource,capture_rate,^solar_.*', 'my')
    columns = [column for column in df.columns if "existing" not in column]
    df = df[columns]
    df.columns = [column.rsplit("_", 1)[1] for column in df.columns]
    plot_definitions = get_plot_definitions(method="line", title=sheet_name, xlabel="year", ylabel="-")
    ...
    dfx = cgriddb.get_value_with_index(ids, 'resource,capacity', 'my')
    dfx = dfx.fillna(0)
    dfx = dfx * 1e-3
    ...
    df1x = cgriddb.get_value_with_index(ids, 'zone,total_demand', 'my')
    df1x = df1x * 1e-6
    df2x = cgriddb.get_value_with_index(ids, 'resource,total_generation', 'my')
    df2x = df2x.fillna(0)
    df2x = df2x * 1e-6
    zones = df1x.columns
    for zone in zones:
    ...
    df1 = cgriddb.get_value_with_index(ids, 'transmission,capacity1', 'my')
    for column in df1.columns:
        for index in df1.index:
            df1.loc[index, column] = read_compressed_data(df1.loc[index, column]).mean()
    df2 = cgriddb.get_value_with_index(ids, 'transmission,capacity2', 'my')
    ...
    df = cgriddb.get_value_with_index(ids, 'transmission,flow', 'my')
    for column in df.columns:
        for index in df.index:
